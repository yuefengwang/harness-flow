...
print("\n── 2. init CLI ──")
cleanup()
run(f"python3 {SW} init --name=test-cli --type=feature --no-tmux", 0, "init with args")
run(f"python3 {SW} init --name=test-cli --no-tmux", 1, "init duplicate name")
...
# ── 5. next ──
print("\n── 5. next ──")
run(f"printf 'y\n' | python3 {SW} next --name=test-cli --no-tmux", 0, "next starts stage")
...
# ── 7. advance --force ──
print("\n── 7. advance --force ──")
run(f"printf 'y\n' | python3 {SW} next --name=test-cli --no-tmux", 0, "next for stage 02")
...
for stage in range(3, 5):
    run(f"printf 'y\n' | python3 {SW} next --name=test-cli --no-tmux", 0, f"next stage {stage}")
...
# Create fresh task
cleanup()
run(f"python3 {SW} init --name=rm-test --no-tmux", 0, "init for remove test")
...
run(f"python3 {SW} init --name=rm-test --no-tmux", 0, "init same name after remove")
...
print("\n── 12. resume ──")
cleanup()
run(f"python3 {SW} init --name=resume-test --no-tmux", 0, "init for resume")

...
from ..core.config import ROOT, WORKFLOW, TASKS, STAGES, STAGE_NAMES, load_harness_config, resolve_agent_type
...
def cmd_advance(args):
    ...
    try:
        ...
        # 强阻断钩子 (Hard Check)
        hook_script = WORKFLOW / "hooks" / f"check_{STAGES[idx]}.sh"
        # 兼容新的命名规范
        if not hook_script.exists():
            hook_script = WORKFLOW / "hooks" / f"post_check_{STAGES[idx]}.sh"

...
### 2.9.17 同一句报错指向三个不同根因：自评被当成硬门禁（任务 maybework / 7090 / ppppp）

**形状**：S11（判据的输入或触发时机由被判者控制）。

**现场**：三个任务在 01 阶段都被同一句话拦死 ——「❌ 产出区未给出歧义分数」，
而三者的真实根因没有一个是「少写了一个数字」：

| 任务 | decisions | 产出区 | 真实根因 |
|---|---|---|---|
| maybework | 3 | 175 字符 | **写了**「歧义分数达到 8」，正则不认「达到」二字 |
| 7090 | 5 | 360 字符 | agent 在正文里问「你同意方案 A 吗」就停了，阶段没走完 |
| ppppp | 14 | 1985 字符 | 问了 14 轮无收敛出口（2.9.10 修了上界，分数仍缺） |

用户侧的观感是**死锁**：门禁要一个分数，agent 已经收尾不再发言，
用户在 TUI 上除了反复敲 `/advance` 拿到同一句报错之外无事可做。
这与 2.9.11（qqqq 的 O6）同型：**拦住一条路而不给替代路径**。

**根因**：歧义分数是 agent 的**自评**，不是 harness 观测到的事件。
判据的输入由被判者提供时，它有一个共同的失败模式 ——「不写就读不到」，
于是门禁只会说「你没写 X」，而真正断掉的往往是上游某个动作。
判据把三种病报成了同一种，还指错了方向。

**修复**（A13 的第 2 步，用户拍板）：

1. **自评降级为参考**：分数缺失或低于 8 都**不再硬拦**，改为记账 + 提示。
   真正拦空转的是**硬规则** —— 产出区实质内容 ≥ 80 字符，
   它量的是 harness 落盘的围栏区（nonce 不可预测），agent 绕不过去。
2. **放行必须留痕**：新增 `stage_state.record_ambiguity` /
   `read_ambiguity_record`，三态 `ok` / `below_threshold` / `unavailable`
   写进 `.state`，门禁输出带 ❓。放行而不记账等于把「没测到」洗成
   「测过了」，那是第 2 节明令禁止的二态化。
3. **正则接纳中文动词**：`达到` / `已达到` / `为` / `是` / `评为`。
   maybework 的 agent 没做错任何事 —— **判据自己的缺陷不该由被判者承担**。
   用动词白名单而不是通配：`.{0,4}` 会把「歧义分数一节里提到 3 个风险」
   读成 3，读到假数字比读不到更危险（两条边界测试钉着这点）。

**S7 自查（本次修复自己引入的）**：留痕写了却无人读，就等于没写。
因此把记录接进 02 的 prompt（`_read_ambiguity_carryover`）：01 自评为
`unavailable` / `below_threshold` 时，02 会被告知「需求可能带着未澄清的
假设，请先用 `question` 澄清」；达标则**不输出任何东西**，避免提示泛滥。
注入的是 harness 自己写的记录，不是 agent 的自述 —— 证据流向下游，
自述留在原地。这一条是 2.9.16 教训的直接应用：**修复引入的新代码
也要过形状库**，而这恰恰最容易跳过，因为注意力都在旧 bug 上。

**显式改判（DEV-PROTOCOL 1.2）**：`test_ambiguity_gate.py` 里
`test_score_below_threshold_blocks` 与 `test_missing_score_is_unavailable_not_pass`
原本断言「硬拦」，本轮**推翻的是结论方向**（拦 → 放行 + 记账），
...
本次只保证它不再死锁：用户现在可以推进，且 `.state` 如实记着这一项没测到。

---

### 2.9.18 阶段完成性：0 轮问答不得过闸（A13 第 1 步，任务 7090 的真实病根）

**形状**：S11（判据的输入或触发时机由被判者控制）—— 这次治的是**时机**那一半。

**现场**：2.9.17 解开了 7090 的死锁（自评降级为参考），但它的**真实**
病根没动：`.state` 里 `stage_status` 至今是 `running`。那个阶段从来没
走完，门禁却已经判过了。链条是：
...
判据的输入由被判者提供已经危险（2.9.17），判据的**触发时机**也由被判者
控制，则门禁只会在错误的时刻测量错误的东西。

**修复**：`output_check._stage_completion_problems` —— 01 阶段
`decisions == 0` 即拦，排在实质内容判据之后、自评提示之前。

**为什么只用 `decisions`（用户拍板）**：只用纯证据，零误报优先。
`record_decision` 只在用户**实际回答**时写入，三个生产调用点
（`ui/tui.py`、`web/engine_manager.py`、`cli/test_cmd.py`）无一例外。
agent 不经手这个字段 —— 伪造不了，也跳不过。

**被否决的方案：问句启发式**。实测产出区结尾：7090 是「你同意方案 A
吗？」、helloworld 是「这两者差异很大」，其余五个任务都是「请输入
`/advance`」—— 信号看着很强。但那仍然是**读 agent 写的文本**，与歧义
分数同一个错。用户在 2.9.17 那轮已经纠正过一次（当时我提议拿「agent 有没有
写『请输入 /advance』」当判据，用户指出 `/advance` 是**用户在 TUI 敲的
命令**，agent 那句话只是正文里的一句话）。判据不该第三次栽在同一个形状上。

**判据的边界**：只查「有没有问过」，**不查「问够几轮」**。
`hook-01-02` 的「≥3 questions」是 prompt 级收敛建议，不是门禁硬规则。
把轮次下界硬化会立刻撞上 2.9.10（ppppp 14 轮无上界）的反面 ——
下界硬化会逼出「为凑数而提问」。

**与原设计的偏离**：A13 第 4.1 节写的是新建 `stage_completion.py`。
实施时并入 `output_check.py`：判据只剩 `decisions == 0` 一条，
为它单开模块会让「01 的准出判据」散在两个文件里，下一个改判据的人
得同时找两处。已在 A13 的范围行注明。

**基线对照实测**（`git archive 9276556` 检出到 /tmp，只搬测试）：
修复前 **2 failed / 6 passed** —— 0 轮问答的任务过了闸，agent 在正文里
声称「已完成 5 轮 question 交互」也过了闸；修复后 8 条全绿。
6 条 passed 的是前提自检与边界（他阶段不受影响、判据不恒真），
证明红来自判据本身。

**真实任务实跑**：7090(5轮) / maybework(3) / ppppp(14) / 8090(7) /
helloworld(1) 全部放行，**零误伤**；手工构造的 0 轮任务被钩子拦下 rc=1。

**存量夹具的连带修改**：`test_ambiguity_gate` / `test_ambiguity_advisory` /
`test_stage_file_tamper` / `test_early_stage_output_check` 四个文件的 01
夹具原先不带 `decisions`，新判据上线后它们的红会来自完成性而不是各自的
被测对象。逐个补上问答记录并注明理由 —— **改的是夹具的前置条件，
不是断言**，被测的形状一个都没动。

**遗留**：7090 那种「问了 5 轮但最后一次追问没走 question 工具」仍抓不住。
要抓它需要 harness 侧的「最后一次发言之后有无 question 事件」，
属会话层事件，本次未做。本判据只保证「一轮都没问过」不能过闸。

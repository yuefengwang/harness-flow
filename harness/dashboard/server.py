...
async def list_projects() -> list[ProjectInfo]:
    projects = [ProjectInfo(name="simple-workflow", path=str(ROOT), has_readme=True)]
    if REPO_DIR.exists():
...
async def platform_status() -> PlatformStatus:
    ...
    return PlatformStatus(
        ...
        projects_count=projects_count + 1,  # +1 for simple-workflow itself
    )


@app.post("/api/dispatch")
async def dispatch_task(req: DispatchRequest) -> DispatchResult:
    if req.project == "simple-workflow":
        pass
